...
# Paths
file_path = "./Ebay_dataset_T-SEE/TSNE_positions.json"
image_folder = "./Ebay_dataset"
output_folder = "./EBAYDataset_TSEE"
...
for i, img_name in enumerate(tsp_path):
    new_name = f"{i:04d}.png"  # Save everything as PNG
    ...
    if os.path.exists(original_path):
        try:
            ...
            img = Image.open(original_path).convert("RGB")  # Convert to RGB (fix mode issues)
...
ffmpeg_json_path = "./ffmpeg_tsp_path2.json"
...
output_video = "./video/out1.mp4"
ffmpeg_command = [
    "ffmpeg",
    "-framerate", "30",  # Set frame rate (adjust as needed)
    "-i", os.path.join(output_folder, "%04d.png"),  # Use PNG format
    "-c:v", "libx264",
    "-crf", "23",
    "-preset", "fast",
    output_video
